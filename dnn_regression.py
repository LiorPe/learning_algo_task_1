...
def train_model(regressor,preprocessed_data_directory):
    global training_set
    global test_set
    ...
    global FEATURES
    global LABEL
    training_set = pd.read_csv(os.path.join(preprocessed_data_directory,'train.csv'))
...
def get_predictions_for_test_set(regressor, preprocessed_data_directory):
    test_set =pd.read_csv(os.path.join(preprocessed_data_directory,'test.csv'))
    ...
    id = 1461
    with open (os.path.join(preprocessed_data_directory,"predictions.txt"), mode='w') as file:
...
def train_and_get_predictions(preprocessed_data_directory):
    regressor = tf.estimator.DNNRegressor(feature_columns=feature_cols,
                                              hidden_units=[1024,512,256],
                                              model_dir=os.path.join(preprocessed_data_directory,'model_dir'),
                                              optimizer=tf.train.ProximalAdagradOptimizer(
                                                  learning_rate=0.05,
                                                  l1_regularization_strength=0.001
                                              ))
    train_model(regressor,preprocessed_data_directory)
    get_predictions_for_test_set(regressor,preprocessed_data_directory)

train_and_get_predictions(preprocessed_data_directory="preprocessed_data/example")